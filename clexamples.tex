% 编译方式：texify --pdf --engine=xetex 或 latexmk -xelatex 
%          或 xelatex -> bibtex -> xelatex*2
% 纸张大小：A4
% 字号：11 pt
% 字体：Fandol（中文逗号，分号，冒号，引号显示成英文样式）
...
\begin{document}
    ...
    \textit{本是楷体的词被\emph{强调}。}

    词被\uline{下划线}。

    \textit{本是楷体的词被\uline{下划线}。}
    ...
    \begin{description}
        ...
        90\textdegree。如果与母球与目标球正碰撞，理想状态下母球会瞬间静止。也叫斯登
        或司登，英文名为 stun。
        \item[推杆] 指以较小的力量击打母球中部，使得母球在接触目标球之前因台面的摩擦作
        用而最终滚向目标球。效果是在母球与目标球接触后，母球与目标球的运动方向的夹角小
        于 90\textdegree。如果与母球与目标球正碰撞，母球之后会向前运动一段距离。
    \end{description}
    \subsection{摘录（quote）}
    \begin{quote}
...
\end{verbatim*}
    ...
    \begin{table}[htbp]
        ...
        \begin{tblr}{c|ccccc}
            ...
            运动员 & 澳网 & 法网 & 温网 & 美网 & 奥运会\\
            \hline
            施特菲\crdot{}格拉芙 & 1988 & 1988 & 1988 & 1988 & 1988 \\
            \hline
        \end{tblr}
        \caption{年度金满贯}
        \label{table} % 标签放这里！
    ...
    \begin{figure}[htbp]
        \centering
        \includegraphics[width=0.6\textwidth]{imgs/FND.jpg}
    ...
    \begin{equation}
        E=mc^2\text{。}\label{eq2}
    \end{equation}

    把行内公式 $E=mc^3$ 写成有特殊编号的独立公式：
    \begin{equation}
        E=mc^3\text{。}\tag{$\ast$} \label{eq3}
    \end{equation}

    式~\eqref{eq2} 显然是对的。式~\eqref{eq3} 连量纲都不对……
    \subsection{文字}
    这样写是错的：
    \begin{equation*}
        x > 0    任意 x \in R_+。
    ...
    这样写才是对的：
    \begin{equation*}
        x > 0 \qquad \text{任意} \; x \in \mathbb{R_+}\text{。}
    \end{equation*}
    \subsection{上下标}
    \begin{equation*}
        \sum_{i=1}^{100} i = \sum^{100}_{j=1} j\text{。}
    \end{equation*}
    \begin{equation*}
        a^x+y \neq a^{x+y}\text{。}
    \end{equation*}
    ...
    \end{equation*}
    \begin{equation*}
        5 \equiv 1 \pmod{2}\text{。}
    \end{equation*}
    \subsection{分数}
    \begin{equation*}
        \frac{\mathrm{d}^{n}y}{\mathrm{d}x^{n}}\text{。}
    \end{equation*}
    \begin{equation*}
        \frac{\partial^{2}z}{\partial{x}\partial{y}}\text{。}
    \end{equation*}
    \subsection{二项式系数}
    \begin{equation*}
    ...
    数学比化学复杂多了：
    \begin{equation*}
        \sum_{\substack{1\le{i}\le{n}\\j>i}} l_{ij}^2 = 0\text{。}
    \end{equation*}
    \subsection{矩阵}
    \begin{equation*}
        \boldsymbol{I} =
    ...
    \begin{align}
        \nabla\frac{1}{r} & = \nabla\frac{1}{\sqrt{x^2+y^2+z^2}} \label{oeq}\\
            & = -\frac{x\boldsymbol{i}+y\boldsymbol{j}+z\boldsymbol{k}}
                      {\sqrt{x^2+y^2+z^2}^3} \nonumber \\
            & = -\frac{\hat{\boldsymbol{r}}}{r^2}\text{。}\tag{$\star$} \label{beq}
    \end{align}

    \eqref{oeq} 能直接从定义得出。最后的结果 \eqref{beq} 非常漂亮！
    \subsection{空格}
    $ \text{这是} \  \text{1 space\text{。}} $

    $ \text{这是} \! \text{-3/18 em\text{。}} $

    $ \text{这是} \, \text{3/18 em\text{。}} $

    $ \text{这是} \: \text{4/18 em\text{。}} $

    $ \text{这是} \; \text{5/18 em\text{。}} $

    $ \text{这是} \quad \text{1 em\text{。}} $

    $ \text{这是} \qquad \text{2 em\text{。}} $
    \subsection{空位}
    ...
    这样写是错的：$X:\Omega\to\mathbb{R}$。

    这样写才是对的：$X\colon\Omega\to\mathbb{R}$。
    \subsection{符号小全}
    这里解决不了的，\href{
